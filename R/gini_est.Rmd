...
```{r}
gini_states = read.csv("gini_states.csv")
head(gini_states)
```
...
```{r}
c(min(gini_states$time), max(gini_states$time))
```

```{r}
empBC = beta_conv(gini_states,
                  time_0 = 2012,
                  time_t = 2021,
                  all_within = FALSE,
                  timeName = 'time')
...
```{r}
mySTB <- sigma_conv(gini_states, timeName="time")
mySTB
...
```{r}
res <- departure_mean(oriTB = gini_states, sigmaTB = mySTB$res)
names(res$res)
...
```{r}
gamma_conv(gini_states,last=2021,ref=2012,timeName="time")
```

### Delta_convergence

```{r}
delta_conv(gini_states,"time")
```

### Scoreboard

```{r}
resTB <- scoreb_yrs(gini_states,timeName = "time")
resTB
```
...
timeName <- "time"
myx_angle <- 45
outSig <- sigma_conv(gini_states, timeName = timeName,
time_0=2012,time_t=2021)
miniY <- min(gini_states[,- which(names(gini_states) == timeName )])
maxiY <- max(gini_states[,- which(names(gini_states) == timeName )])
estrattore<- gini_states[,timeName] >= 2012 & gini_states[,timeName] <= 2021
ttmp <- cbind(outSig$res, dplyr::select(gini_states[estrattore,], -contains(timeName)))
...
```{r}
obe_lvl <- scoreb_yrs(gini_states, timeName = timeName)$res$sco_level_num
