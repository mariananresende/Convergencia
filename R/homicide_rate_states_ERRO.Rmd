...
```{r}
homicide_rate_states = read.csv("homicide_rate_states.csv")
head(homicide_rate_states)
```
...
```{r}
c(min(homicide_rate_states$time), max(homicide_rate_states$time))
```

```{r}
empBC = beta_conv(homicide_rate_states,
                  time_0 = 1980,
                  time_t = 2021,
                  all_within = FALSE,
                  timeName = 'time')
...
```{r}
mySTB <- sigma_conv(homicide_rate_states, timeName="time")
mySTB
...
```{r}
res <- departure_mean(oriTB = homicide_rate_states, sigmaTB = mySTB$res)
names(res$res)
...
```{r}
gamma_conv(homicide_rate_states,last=2021,ref=1980,timeName="time")
```

### Delta_convergence

```{r}
delta_conv(homicide_rate_states,"time")
```

### Scoreboard

```{r}
resTB <- scoreb_yrs(homicide_rate_states,timeName = "time")
resTB
```
...
timeName <- "time"
myx_angle <- 45
outSig <- sigma_conv(homicide_rate_states, timeName = timeName,
time_0=1980,time_t=2021)
miniY <- min(homicide_rate_states[,- which(names(homicide_rate_states) == timeName )])
maxiY <- max(homicide_rate_states[,- which(names(homicide_rate_states) == timeName )])
estrattore<- homicide_rate_states[,timeName] >= 1980 & homicide_rate_states[,timeName] <= 2021
ttmp <- cbind(outSig$res, dplyr::select(homicide_rate_states[estrattore,], -contains(timeName)))
...
```{r}
obe_lvl <- scoreb_yrs(homicide_rate_states, timeName = timeName)$res$sco_level_num
